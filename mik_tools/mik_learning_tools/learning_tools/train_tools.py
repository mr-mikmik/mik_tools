...
def train_model(args, Model, model_args, data_name, dataset, train_loader, val_loader=None, initialize_model_weights_version=None, resume_training_version=None, callbacks=None):
    data_sizes = dataset.get_sizes()

    if resume_training_version is not None:
        model = load_model(Model, load_version=resume_training_version, data_path=data_name,
