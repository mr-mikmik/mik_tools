...
class ImageEncoder(nn.Module):
    ...
    def forward(self, x):
        in_size = x.shape # shape (Batch_size, ..., C_in, H_in, W_in)
        x = torch.flatten(x,start_dim=0, end_dim=-4) # (BatchExtendedSize, C_in, H_in, W_in)
        conv_out = self.conv_encoder(x) # shape (Batch_size, ..., C_out, H_out, W_out)
        fc_in = torch.flatten(conv_out, start_dim=-3) # flatten the last 3 dims (BatchExtendedSize, C_out*H_out*W_out)
        z = self.fc_encoder(fc_in)
        z = z.reshape(*in_size[:-3], z.shape[-1]) # unbatch
        return z

    def _get_convs_h_sizes(self, num_convs, conv_h_sizes):
        if conv_h_sizes is None:
            hidden_dims = [self.input_size[-3]] + [self.input_size[-3]*3]*num_convs
        else:
            hidden_dims = [self.input_size[-3]] + conv_h_sizes
    ...
    def _get_conv_encoder(self):
        ...
        for i, h_dim in enumerate(self.hidden_dims[:-1]):
            ...
            size_i = (np.floor((sizes[-1][-2:]+2*padding - dilation*(ks-1) - 1)/stride) + 1).astype(np.int64)
            size_i = np.floor_divide(size_i, pooling_size_i).astype(np.int64)
            sizes.append(size_i)
        conv_encoder = nn.Sequential(*conv_modules)
        conv_img_out_size_wh = sizes[-1][-2:]
